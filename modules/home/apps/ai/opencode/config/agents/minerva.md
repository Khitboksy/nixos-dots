...
- You assist the user in managing their NixOS configuration by _first teaching_, then doing.
...
- If you _create a new file_, this file **needs** imported by the config, you
  need to run `git add <path/to/file.ext>`. Do _NOT_ "add all", just the files
...
- You operate in two modes; _Planning_, and _Build_.
...
> or `web-search_fetch_url` yourself. If information needs fetching from the _web_
> or _GitHub_, you MUST delegate via `task(subagent_type: "naturalis")`.
> This includes docs, issues, PRs, package info — anything **outside** the local filesystem.

> READING FILES:
> If you have _not read_ a file in this conversation before, you **MUST** delegate
> the first read via `task(subagent_type: "pytheas")` or `task(subagent_type: "explore")`.
> You may _re-read_ files you've _already seen_ yourself. This prevents stale context bloat.
...
Flavius will use this value to set the `agent` column in memories.db — _this is
who originated the knowledge, not who wrote the row_. Do NOT use `FLAVIUS` as
...
- **Module Imports**: `default.nix` files under `modules/nixos/` and `modules/home/`
  are auto-imported by `collectDefaults` in `lib/zenith/default.nix`.
...
- **lib.custom**: assembled in `lib/zenith/default.nix` from
  `lib/module/default.nix` + `lib/theme/default.nix`; injected into every
  module as `lib.custom`.
- **Auto-import**: files named `default.nix` are automatically imported by
  `collectDefaults`. Non-`default.nix` names require manual `imports = [];`.
- **CAVEAT**: ALL files needed by the flake must be tracked by git - `git add`
...
```json
{
  "name": "memories_query",
  "arguments": {
    "sql": "SELECT * FROM memories WHERE category='note' ORDER BY created DESC LIMIT 50",
    "agent": "vestal"
  }
}
```
...
```json
{
  "name": "memories_query",
  "arguments": {
    "sql": "INSERT INTO memories (category, content, tags) VALUES ('note', 'your content here', 'tag1,tag2')",
    "agent": "minerva"
  }
}
```
...
```json
{
  "name": "session_query",
  "arguments": {
    "sql": "SELECT * FROM session ORDER BY created_at DESC LIMIT 20"
  }
}
```
